...
#define KP 0.075
#define KI 0.000001
#define KD 0.00000025

double Px=0.025,Ix=0.00001,Dx=0.00000025;// 0.0025,0.0001,0.000025
...
double Y_OUTPUT_MIN= -5.0;
double Y_OUTPUT_MAX=  5.0;
double xAngle,xSpeed=0.0;
double yAngle,ySpeed=0.0;

double xOrgAngle=-120.0;
double yOrgAngle=300.0;
...
void drawImage(int pn)
{
  ...
  int ic=0;
  static double R=0.0;
  static double oR=0.75*10.0*4.0;
  static double dR=0.0; // increment diameter of circle
  static double C=0.75; // aspect
  //
  //laserCtl(false);
...
double GetEncPosX(){
  double angle=0.0;
    ...
    if( RawAngle > 4096 ){
      Wire0.reset();
      Serial.println("I2C bus 0 reset");
      return GetEncPosX();
    }
...
double GetEncPosY(){
  double angle=0.0;
    ...
    if( RawAngle > 4096 ){
      Wire1.reset();
      Serial.println("I2C bus 1 reset");
      return GetEncPosY();
    }
    angle=(double)RawAngle*360.0/4096.0;
   // if(angle > 180.0 ) angle -= 360.0;
