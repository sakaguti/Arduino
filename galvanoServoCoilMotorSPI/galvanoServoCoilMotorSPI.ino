...
double Px=0.1,Ix=0.0001,Dx=0.000001;// 0.0025,0.0001,0.000025
...
double xOrgAngle=200.0;
double yOrgAngle=50.0;

double xSetAngle=xOrgAngle, xSetAngleDt=1.0, xSetAngleOld=0.0;
double ySetAngle=yOrgAngle, ySetAngleDt=1.0, ySetAngleOld=0.0;
...
int imgCount=0;
#define WNO  4000 // drawing speed. motor is 4000 max, coil is 400 max.
#define WNO2 250  //
...
#ifdef GAIN_ADJUST
double GainAdjust(double v, double vOld)
{
    double x;
    x=abs(v-vOld);
    return 1.0-0.75/(1.0+exp(-4.0*(x-1.0)))+0.1;
}
#endif


void ComputeXY(double xAngle, double xSetAngle,double yAngle, double ySetAngle, double *xSpeed, double *ySpeed)
{
   // How long since we last calculated
   unsigned long now = micros();
   double timeChangeX = (double)(now - lastTimeM1)/1000000.0;
   double timeChangeY = (double)(now - lastTimeM2)/1000000.0;
   // Compute all the working error variables
   double errorX = xSetAngle - xAngle;
   double errorY = ySetAngle - yAngle;
   errSumM1 += (errorX * timeChangeX);
   errSumM2 += (errorY * timeChangeY);
   double dErrX = (errorX - lastErrM1) / timeChangeX;
   double dErrY = (errorY - lastErrM2) / timeChangeY;
  ...
  #endif
#ifdef GAIN_ADJUST
   xSetAngleDt=GainAdjust(xSetAngle,xSetAngleOld);
   ySetAngleDt=GainAdjust(ySetAngle,ySetAngleOld);
#endif
 
   // Compute PID Output
   *xSpeed = xSetAngleDt*ySetAngleDt * (Px * errorX + Ix * errSumM1 + Dx * dErrX);
   *ySpeed = xSetAngleDt*ySetAngleDt * (Py * errorY + Iy * errSumM2 + Dy * dErrY);
...
double Computey(double yAngle, double ySetAngle)
{
   ...
   ySpeed = ySetAngleDt*(Py * error + Iy * errSumM2 + Dy * dErr);
...
void laserCtl(double laser_pwm)
{
  if(laser_pwm>0.0){
    // laser ON
...
//
  ...
  ledcSetup(LASER_CHANNEL, 5000, 16);
 ...
 */
  // SPI　(vSPI)
  SPI.begin(18, 19, 23, 5);//int8_t sck, int8_t miso, int8_t mosi, int8_t ss
...
// read UART command if avairable
  ...
  if(sSW==false){
    laser_pwm=0.0;
    errSumM1=0.0; lastErrM1=0.0;
    errSumM2=0.0; lastErrM2=0.0;
    }
...
*/
  ComputeXY(xAngle, xSetAngle, yAngle, ySetAngle, &xSpeed, &ySpeed);
