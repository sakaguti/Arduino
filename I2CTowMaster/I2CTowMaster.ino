...
/*
// I2C master 2
    Wire2.begin(5,4); // join i2c bus (address optional for master)
    Wire2.setClock(400000);
*/

/*
// PWM
    PWM::pwm_init(1000, duty,4,io_info);

// Set duty, the max duty is 8191 which is set in pwm.h
uint32 channel = 1;
uint32 period=32767;

    PWM::pwm_set_duty(duty[0], channel);   // 0~3
    PWM::pwm_set_period(period);  // set period;
    PWM::pwm_start();
  */
}
...
/*
    ...
    // Read RAW_ANGLE value from encoder
    Wire2.beginTransmission(AS5600_AS5601_DEV_ADDRESS);
    Wire2.write(AS5600_AS5601_REG_RAW_ANGLE);
    Wire2.endTransmission(false);
    Wire2.requestFrom(AS5600_AS5601_DEV_ADDRESS, 2);
    uint16_t RawAngle2 = 0;
    RawAngle2  = ((uint16_t)Wire2.read() << 8) & 0x0F00;
    RawAngle2 |= (uint16_t)Wire2.read();
    // Raw angle value (0 ~ 4095) is stored in RawAngle
 */
